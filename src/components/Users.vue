      ...
      <div class="card">
        <div class="card-header form-inline">
          <div class="float-left form-inline">
            ...
            <select
              ...
              @change="$emit('perPageOptionChanged')"
            >
              <option
                v-for="pageNo in perPageDropDown"
                :key="pageNo.id"
                :value="pageNo"
                >{{ pageNo }}</option
              >
          ...
          <div class="card-tools float-right">
            <button
              class="btn btn-default mb-2 mr-sm-2 bg-green"
              :disabled="
                Object.entries(tableData.search).length === 0 &&
                  tableData.search.constructor === Object
              "
              data-toggle="tooltip"
              title="Search"
              @click="$emit('searched')"
            ...
            <button
              class="btn btn-default mb-2 mr-sm-2 bg-red"
              :disabled="
                Object.entries(tableData.search).length === 0 &&
                  tableData.search.constructor === Object
              "
            ...
            >
              <i class="fas fa-power-off"></i>&nbsp; Reset Search
            </button>
            <button
              type="submit"
        ...
        <div class="card-body table-responsive">
          ...
          >
            <tbody>
              ...
              <tr v-for="user in users" :key="user.id">
                ...
                <td>
                  <button class="btn btn-success" @click="editModal(user)">
                    <i class="fas fa-edit"></i>
                  </button>
                  <button class="btn btn-danger" @click="destroyData(user.id)">
          ...
          <app-pagination
            ...
            @pageClicked="page_url => $emit('pageChanged', page_url)"
          ></app-pagination>
        </div>
        <!-- /.card-body -->
      </div>
      ...
      <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 v-show="!editMode" class="modal-title" id="ModalTitle">
              Add User
            </h5>
            <h5 v-show="editMode" class="modal-title" id="ModalTitle">
              Update User
  ...
  methods: {
    ...
    getUsers(url = this.api_url) {
      ...
      axios
        ...
        .catch(errors => {
          Swal.fire("Failed!", "Document Loading has been failed", "warning");
          this.$Progress.fail();
        });
    },
    ...
    getRelatedResources() {
      this.$Progress.start();
      this.form
        .get("api/fetchUserRelatedModels")
        .then(({ data }) => {
      ...
      Swal.fire({
        ...
        if (result.value) {
          axios
            ...
            .catch(() => {
              Swal.fire("Failed!", "Something has been wrong", "warning");
              this.$Progress.fail();
            });
        }
      });
